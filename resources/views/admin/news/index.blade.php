...
<x-layouts::app :title="__('Manage News')">
    <div class="min-h-screen bg-gray-900 py-8 px-4 sm:px-6 lg:px-8">
        <div class="max-w-7xl mx-auto">
            <!-- Header -->
            <div class="mb-8 flex justify-between items-center">
                <div>
                    <h1 class="text-3xl font-bold text-gray-100 mb-2">Manage News</h1>
                    <p class="text-gray-400">Create and manage your news articles</p>
                </div>
                <a href="{{ route('admin.news.create') }}" class="inline-flex items-center gap-2 px-6 py-3 text-sm font-medium text-black bg-[#fbbf24] hover:bg-amber-500 rounded-lg shadow-sm hover:shadow-md transition-all duration-200">
                ...
                <div class="overflow-x-auto">
                    <table class="w-full">
                        ...
                        <tr>
                            ...
                            <th class="px-6 py-4 text-left text-xs font-medium text-gray-400 uppercase tracking-wider">Published</th>
                            ...
                            <tr class="hover:bg-gray-700/50 transition-colors">
                                <td class="px-6 py-4">
                                    @if($item->featured_image)
                                        <img src="{{ Storage::url($item->featured_image) }}" alt="{{ $item->title }}" class="w-12 h-12 object-cover rounded-lg">
                                    @else
                                ...
                                <td class="px-6 py-4">
                                    <div class="text-sm font-medium text-gray-200">{{ $item->title }}</div>
                                    <div class="text-xs text-gray-400">Slug: {{ $item->slug }}</div>
                                </td>
                                ...
                                <td class="px-6 py-4 text-sm text-gray-300">{{ $item->published_at ? $item->published_at->format('M d, Y') : '-' }}</td>
                                <td class="px-6 py-4 text-right">
                                    <div class="flex items-center justify-end gap-2">
                                        <a href="{{ route('admin.news.edit', $item) }}" class="text-blue-400 hover:text-blue-300 transition-colors" title="Edit">
                                        ...
                                        <form action="{{ route('admin.news.destroy', $item) }}" method="POST" class="inline">
                                            @csrf
                                            @method('DELETE')
                                            <button type="submit" class="text-red-400 hover:text-red-300 transition-colors" title="Delete" onclick="return confirm('Are you sure you want to delete this news article?')">
                                                <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        ...
                        @empty
                            <tr>
                                <td colspan="7" class="px-6 py-12 text-center text-gray-400">
                                    No news articles found. Click "Create New Article" to get started.
                                </td>
                            </tr>
                        @endforelse
                        </tbody>
